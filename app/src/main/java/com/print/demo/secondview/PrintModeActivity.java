...
public class PrintModeActivity extends BaseActivity implements OnClickListener {
    ...
    private Button btnBlackOn, btnBlackOff;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnBlackOn = (Button) findViewById(R.id.button_black_mark_on);
        btnBlackOn.setOnClickListener(this);
        btnBlackOff = (Button) findViewById(R.id.button_black_mark_off);
        btnBlackOff.setOnClickListener(this);
    ...
    private void setBlackMark(boolean isOn) {
        if (isOn) {
            mContext.getObject().ASCII_PrintBuffer(mContext.getState(), new byte[]{0x1B, 0x06},
                    2);
            int printBuffer = mContext.getObject().ASCII_PrintBuffer(mContext.getState(), new byte[]{0x1B, 0x2B, 0x01},
                    3);
            Log.d("ZM", "setBlackMarkOn: " + printBuffer);
            mContext.getObject().ASCII_PrintBuffer(mContext.getState(), new byte[]{0x1B, 0x16},
                    2);
        } else {
            mContext.getObject().ASCII_PrintBuffer(mContext.getState(), new byte[]{0x1B, 0x06},
                    2);
            int printBuffer = mContext.getObject().ASCII_PrintBuffer(mContext.getState(), new byte[]{0x1B, 0x2B, 0x00},
                    3);
            Log.d("ZM", "setBlackMarkOff: " + printBuffer);
            mContext.getObject().ASCII_PrintBuffer(mContext.getState(), new byte[]{0x1B, 0x16},
                    2);
        }
